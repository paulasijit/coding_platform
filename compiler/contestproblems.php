...
?>
				<?php if(trim(isset($_POST['cname']))==""){
					if(isset($_GET['success'])){
		$sql = "SELECT cname FROM problems where sl='".$_GET['success']."'";
            $res = mysqli_query($db,$sql);
			$r = mysqli_fetch_array($res);
		}			?>

					<div class="div-profile"><center><p class="impact">CONTEST NAME: <?php echo $r['cname']?></p></center></div><?php
				}else{ ?>
			 <div class="div-profile"><center><p class="impact">CONTEST NAME: <?php echo $_POST['cname']?></p></center></div>
			 <?php
				}
				?>
      ...
      <ul class="nav nav-list">
        ...
        if(isset($_POST['action'])){
            if($_POST['action']=='contest') {
                ...
                if(trim($_POST['cname']) == "")
                    header("Location: contestsolve.php?derror=1");
                else {
			// list all the problems from the database
        	$query = "SELECT * FROM problems where cname='".$_POST['cname']."'";
